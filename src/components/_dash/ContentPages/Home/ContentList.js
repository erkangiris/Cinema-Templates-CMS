...
export default function UserListView({data}) {
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      WebServices.DeleteContentPage({Id:id})
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.managamet.content.edit(id));
    },
    [router]
  );

  const handleTranslateEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.managamet.content.translate(id));
    },
    [router]
  );
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <UserTableRow
                        ...
                        onTranslateEditRow={() => handleTranslateEditRow(row.id)}
...
function applyFilter({ inputData, comparator, filters }) {
  const { contentTitle, status, role } = filters;
  ...
  if (contentTitle) {
    inputData = inputData.filter(
      (user) => user.contentTitle.toLowerCase().indexOf(contentTitle.toLowerCase()) !== -1
    );
  }
