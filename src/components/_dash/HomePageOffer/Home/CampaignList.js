...
export default function UserListView({data}) {
  ...
  const TABLE_HEAD = [
    { id: 'name', label:  t('name'), width: 150 },
    { id: 'date', label:  t('date'), width: 180 },
    { id: 'img', label:  t('img'), width: 220 },
    { id: '', width: 88 },
  ];
  ...
  const handleDeleteRow = useCallback(
    (id) => {
      ...
      WebServices.DeleteCampaign({Id:id})
...
function applyFilter({ inputData, comparator, filters }) {
  const { campaignName, status, role } = filters;
  ...
  if (campaignName) {
    inputData = inputData.filter(
      (user) => user.campaignName.toLowerCase().indexOf(campaignName.toLowerCase()) !== -1
    );
  }
